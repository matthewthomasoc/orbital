...
class body:
	def __init__(self, a, e, orbit_period, inclination, apsidal_prec_period):
		self.a = a
		self.e = e
	...
	def rodrigues(self, axis, vector, theta):
		result = 0;
	...
	def update(self, dt):
		...
		self.orbit_axis = self.rodrigues(prec_axis, self.orbit_axis, dtheta_apsidal)

		# Update orbit periapsis unit vector
		self.periapsis_unit = self.rodrigues(self.orbit_axis, self.periapsis_unit, dtheta_apsidal)
		self.periapsis = self.periapsis_unit * self.rp

		# Update orbit position unit vector
		self.position_unit = self.rodrigues(self.orbit_axis, self.periapsis_unit, self.theta_orbit)
